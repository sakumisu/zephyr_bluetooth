...
struct bt_dev bt_dev = {
    ...
    .le.pkts 	   = Z_SEM_INITIALIZER(bt_dev.le.pkts, 0, 1),
	.le.acl_pkts   = Z_SEM_INITIALIZER(bt_dev.le.acl_pkts, 0, 1),
#endif
#if defined(CONFIG_BT_ISO)
	.le.iso_pkts   = Z_SEM_INITIALIZER(bt_dev.le.iso_pkts, 0, 1),
#endif
#if defined(CONFIG_BT_BREDR)
	.br.pkts       = Z_SEM_INITIALIZER(bt_dev.br.pkts, 0, 1),
#endif
};
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	/* TX thread */
	k_thread_create("BT TX", tx_thread_stack,
			K_KERNEL_STACK_SIZEOF(tx_thread_stack),
			hci_tx_thread, NULL, K_PRIO_COOP(CONFIG_BT_HCI_TX_PRIO));

#if !defined(CONFIG_BT_RECV_IS_RX_THREAD)
	/* RX thread */
	k_thread_create("BT RX", rx_thread_stack,
			K_KERNEL_STACK_SIZEOF(rx_thread_stack),
			(k_thread_entry_t)hci_rx_thread, NULL, K_PRIO_COOP(CONFIG_BT_RX_PRIO));
#endif
